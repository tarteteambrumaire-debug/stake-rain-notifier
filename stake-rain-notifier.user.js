...
(function () {
  ...
  var LANGS = {
    fr: {
      ...
      step1_title:      '👤 Étape 1/4 - Ton pseudo Stake',
      step1_desc:       'Entre ton pseudo Stake pour recevoir les notifications de rains et de mentions.',
      step1_chat:       '💬 Ton chat préféré sur Stake',
      step1_warning:    '⚠️ Important : Laisse le script actif sur ton navigateur !',
      step2_title:      '🔒 Étape 2/4 - Telegram personnel (optionnel)',
      step2_desc:       'Tu peux configurer ton propre bot Telegram pour des notifications privées. Si tu ne sais pas, clique sur "Passer".',
      step3_title:      '⚙️ Étape 3/4 - Choisis tes onglets',
    ...
    en: {
      ...
      step1_warning:    '⚠️ Important: Keep the script active in your browser!',
      ...
      step3_title:      '⚙️ Step 3/4 - Choose your tabs',
  ...
  var CHAT_OPTIONS = [
    ...
    { id: '8590c8cd-65b2-45bd-ab58-973761efd1c6', label: '🇧🇷 Portuguese (BR)' },
  ...
  function recordRain(sender, parsed) {
    ...
    // Dedup persistee : verifier dans le storage pour survivre au refresh
    var rainDedup = load('srn_rain_dedup', {});
    var now = Date.now();
    // Nettoyer les entrees vieilles de plus de 30 minutes
    Object.keys(rainDedup).forEach(function(k){ if (now - rainDedup[k] > 30*60*1000) delete rainDedup[k]; });
    if (rainDedup[rainKey]) return; // Rain deja enregistree
    rainDedup[rainKey] = now;
    save('srn_rain_dedup', rainDedup);
    // Dedup memoire aussi pour les doublons rapides
    if (rainKey === lastRainKey && now - lastRainTime < 10000) return;
    lastRainKey = rainKey; lastRainTime = now;
    var now = Date.now();
...
'.srn-tick-down{color:#ff4444}',
      ...
      '<div id="srn-setup-box">',
        ...
        '<div class="srn-setup-step active" id="sstep-0">',
          ...
          '<div style="display:flex;gap:10px;margin-top:10px">',
            '<button class="srn-setup-btn primary" id="ssetup-lang-en" style="font-size:18px">🇬🇧 English</button>',
            '<button class="srn-setup-btn primary" id="ssetup-lang-fr" style="font-size:18px">🇫🇷 Français</button>',
          '</div>',
        '</div>',
  ...
  function renderTabsToggles() {
    ...
    el.innerHTML = tabs.map(function(t) {
      ...
      var dragHandle = isDash ? '' : '<span class="srn-drag-handle" style="cursor:grab;color:#8899aa;font-size:14px;margin-right:8px;flex-shrink:0" title="Glisser pour réordonner">☰</span>';
      return '<div data-tabid="' + t.id + '" draggable="' + (!isDash) + '" style="display:flex;align-items:center;justify-content:space-between;background:#162330;border:1px solid ' + (on ? '#00d4ff44' : '#1e3a4a') + ';border-radius:8px;padding:7px 10px;font-size:12px;color:' + txtColor + ';font-weight:' + (on ? '600' : '400') + ';transition:opacity .15s">'
        + '<div style="display:flex;align-items:center;flex:1">' + dragHandle + '<span class="srn-tab-label">' + t.label + '</span></div>'
        + '<div class="srn-tab-toggle" style="width:30px;height:16px;border-radius:999px;background:' + bg + ';position:relative;flex-shrink:0;cursor:' + (isDash?'default':'pointer') + '">'
        + '<div style="width:12px;height:12px;border-radius:50%;background:#fff;position:absolute;top:2px;left:2px;transform:' + dot + ';transition:transform .15s"></div>'
        + '</div>'
        + '</div>';
    }).join('');
    ...
    var dragSrc = null;
    el.querySelectorAll('[draggable="true"]').forEach(function(row) {
      ...
      row.addEventListener('dragend', function() {
        row.style.opacity = '1';
        el.querySelectorAll('[data-tabid]').forEach(function(r){ r.classList.remove('srn-drag-over'); r.style.borderColor=''; });
      });
      row.addEventListener('dragover', function(e) {
        e.preventDefault();
        e.dataTransfer.dropEffect = 'move';
        if (row !== dragSrc) row.style.borderColor = '#00d4ff';
      });
      ...
      row.addEventListener('drop', function(e) {
        ...
        var rows = Array.from(el.querySelectorAll('[data-tabid]'));
        var ids = rows.map(function(r){ return r.getAttribute('data-tabid'); });
        var fromIdx = ids.indexOf(dragSrc.getAttribute('data-tabid'));
        var toIdx = ids.indexOf(row.getAttribute('data-tabid'));
        ids.splice(fromIdx, 1);
        ids.splice(toIdx, 0, dragSrc.getAttribute('data-tabid'));
        // Sauvegarder le nouvel ordre
        save(SK_TABS_ORDER, ids);
        // Réordonner ALL_TABS
        var map = {}; ALL_TABS.forEach(function(t){ map[t.id]=t; });
        ALL_TABS.length = 0;
        ids.forEach(function(id){ if(map[id]) ALL_TABS.push(map[id]); });
        renderTabsToggles();
        applyTabsConfig();
      });
    });
