...
(function () {
  ...
  function scanForBet(obj, parentObj) {
    ...
    // Filtre : seulement les bets de l'utilisateur connecte
    var betUser = (obj.user && (obj.user.name || obj.user.username)) || null;
    var myUser = (CONFIG.YOUR_USERNAME || '').toLowerCase().trim();
    // Si pas de username configure, on n'enregistre rien (evite les bets des autres)
    if (!myUser) return;
    // Si le bet a un user et que ce n'est pas le notre, on ignore
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.0.7'; // Doit correspondre a @version
