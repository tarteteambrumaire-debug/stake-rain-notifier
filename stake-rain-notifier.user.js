...
// @version      1.4.15
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.4.15 - Anti-doublons fbPull
// @author       alleluiateam
...
(function () {
        ...
        if (data[key] !== undefined && data[key] !== null) {
          ...
          if (Array.isArray(data[key]) && Array.isArray(local)) {
            ...
            data[key].forEach(function(e) {
              ...
              // Skip si déjà reçu via bridge (même sender+amount+chatId dans les 60s)
              var dedupKey = (e.sender||'') + '|' + (e.amount||'') + '|' + (e.chatId||'');
              if (rainDedup[dedupKey] && now2 - rainDedup[dedupKey] < 60000) return;
              merged.push(e);
              localKeys.push(k);
            });
  ...
  function recordRain(sender, parsed) {
    // Déduplication basée sur sender + amount + chatId dans une fenêtre de 15s
    var rainKey = (parsed.senderFromText || sender || '') + '|' + (parsed.amount || '') + '|' + (parsed.chatId || parsed.recipients.slice(0,2).join(','));
    var rainDedup = load('srn_rain_dedup', {});
    var now = Date.now();
    ...
    if (rainDedup[rainKey] && now - rainDedup[rainKey] < 15000) return; // Rain deja enregistree dans les 15s
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.4.15'; // Doit correspondre a @version
