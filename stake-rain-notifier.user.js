...
'.srn-tick-down{color:#ff4444}',
  ...
  // Styles Bonus Hunt
  var BH_STYLES = [
    '#bh-win{position:fixed;z-index:99999;background:#0f1923;border:1px solid #1e3a4a;border-radius:12px;',
    'box-shadow:0 4px 24px rgba(0,0,0,0.6);font-family:sans-serif;font-size:13px;color:#cdd9e5;',
    'min-width:300px;min-height:200px;width:700px;display:none}',
    '#bh-win.bh-visible{display:flex;flex-direction:column}',
    '#bh-titlebar{padding:10px 14px;background:#162330;border-bottom:1px solid #1e3a4a;border-radius:12px 12px 0 0;',
    'display:flex;align-items:center;justify-content:space-between;cursor:move;user-select:none;flex-shrink:0}',
    '#bh-titlebar-title{font-weight:700;color:#fff;font-size:14px}',
    '#bh-titlebar-btns{display:flex;gap:6px}',
    '#bh-win .bh-tbtn{background:none;border:1px solid #1e3a4a;border-radius:4px;color:#8899aa;cursor:pointer;',
    'font-size:12px;padding:2px 8px}',
    '#bh-win .bh-tbtn:hover{background:#1e3a4a;color:#fff}',
    '#bh-content{padding:14px;overflow-y:auto;flex:1}',
    '#bh-content.bh-minimized{display:none}',
    '.bh-stat-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:12px}',
    '.bh-stat-card{background:#162330;border-radius:8px;padding:10px 12px}',
    '.bh-stat-label{font-size:11px;color:#8899aa;margin-bottom:4px}',
    '.bh-stat-val{font-size:18px;font-weight:700;color:#fff}',
    '.bh-stat-val.green{color:#00ff88}.bh-stat-val.red{color:#ff4444}.bh-stat-val.purple{color:#9f7fef}',
    '.bh-two-col{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px}',
    '.bh-box{background:#162330;border-radius:8px;padding:12px}',
    '.bh-box-title{font-size:11px;color:#8899aa;margin-bottom:8px;font-weight:700;text-transform:uppercase;letter-spacing:.5px}',
    '.bh-be-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}',
    '.bh-be-item{background:#0f1923;border-radius:6px;padding:8px 10px}',
    '.bh-be-label{font-size:11px;color:#8899aa;margin-bottom:2px}',
    '.bh-be-val{font-size:16px;font-weight:700;color:#fff}',
    '.bh-form-row{display:grid;grid-template-columns:2fr 1.2fr .8fr auto;gap:8px;margin-bottom:12px;align-items:end}',
    '.bh-lbl{font-size:11px;color:#8899aa;margin-bottom:4px}',
    '.bh-inp{width:100%;background:#162330;border:1px solid #1e3a4a;border-radius:6px;color:#cdd9e5;padding:6px 10px;font-size:13px;outline:none}',
    '.bh-inp:focus{border-color:#00d4ff55}',
    '.bh-inp[readonly]{color:#8899aa}',
    '.bh-btn2{background:#162330;border:1px solid #1e3a4a;border-radius:6px;color:#cdd9e5;padding:6px 12px;cursor:pointer;font-size:13px;white-space:nowrap}',
    '.bh-btn2:hover{background:#1e3a4a}',
    '.bh-btn2.primary{background:#00d4ff22;border-color:#00d4ff55;color:#00d4ff}',
    '.bh-btn2.danger{background:#ff444422;border-color:#ff444455;color:#ff4444}',
    '.bh-ac-wrap{position:relative}',
    '.bh-ac-list{position:absolute;top:100%;left:0;right:0;background:#162330;border:1px solid #1e3a4a;',
    'border-radius:6px;z-index:999999;max-height:180px;overflow-y:auto;margin-top:2px}',
    '.bh-ac-item{padding:6px 10px;cursor:pointer;display:flex;align-items:center;gap:8px}',
    '.bh-ac-item:hover{background:#1e3a4a}',
    '.bh-ac-img{width:30px;height:30px;border-radius:4px;object-fit:cover;flex-shrink:0;background:#0f1923}',
    '.bh-ac-name{font-size:13px;color:#cdd9e5;font-weight:500}',
    '.bh-ac-provider{font-size:11px;color:#8899aa}',
    '.bh-table{width:100%;border-collapse:collapse;font-size:12px}',
    '.bh-table th{padding:6px 10px;text-align:left;color:#8899aa;font-weight:700;border-bottom:1px solid #1e3a4a;font-size:11px}',
    '.bh-table td{padding:7px 10px;border-bottom:1px solid #1e3a4a15;color:#cdd9e5}',
    '.bh-table tr:last-child td{border-bottom:none}',
    '.bh-table tr:hover td{background:#162330}',
    '.bh-badge{display:inline-flex;align-items:center;gap:3px;padding:2px 7px;border-radius:4px;font-size:11px;font-weight:700}',
    '.bh-badge.pending{background:#ffd70022;color:#ffd700}',
    '.bh-badge.done{background:#00ff8822;color:#00ff88}',
    '.bh-slot-img{width:26px;height:26px;border-radius:3px;object-fit:cover;vertical-align:middle;margin-right:5px}',
    '.bh-empty-row{padding:16px;text-align:center;color:#8899aa}',
    '.bh-sec-title{font-size:11px;color:#8899aa;font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px}',
    '#bh-resize{position:absolute;bottom:0;right:0;width:16px;height:16px;cursor:se-resize;opacity:.4}',
    '#bh-resize:hover{opacity:1}',
  ].join('');
      ...
      '<div id="srn-body">',
        ...
        '<div class="srn-sec" id="tab-bonushunt">',
          ...
          '<div style="text-align:center;padding:20px 0">',
            '<button id="srn-bh-launch" style="background:#00d4ff22;border:1px solid #00d4ff55;border-radius:8px;color:#00d4ff;cursor:pointer;font-size:14px;font-weight:700;padding:12px 28px">🎰 Lancer le Bonus Hunt</button>',
            '<div style="margin-top:10px;font-size:12px;color:#8899aa">Ouvre une fenêtre flottante déplaçable</div>',
          '</div>',
        '</div>',
  ...
  function refreshPanel() {
    ...
    if (curTab === 'bonushunt')  initBonusHuntTab();
  ...
  // ══ BONUS HUNT ═══════════════════════════════════════════════════════════════
  var bhSlots = [], bhBonuses = [], bhNextId = 1, bhSelectedThumb = '';
  ...
  function bhFmt(n) {
    var sym = document.getElementById('bh-currency-sel') ? document.getElementById('bh-currency-sel').value : '€';
    return n.toFixed(2).replace('.', ',') + ' ' + sym;
  }
  function bhFmtX(n) { return n.toFixed(1).replace('.', ',') + '×'; }

  function initBonusHuntTab() {
    var btn = document.getElementById('srn-bh-launch');
    if (btn && !btn._bhInit) {
      btn._bhInit = true;
      btn.addEventListener('click', bhOpenWindow);
    }
  }
      ...
      onload: function(res) {
        try {
          ...
          if (d.success && d.data) {
            ...
            var el = document.getElementById('bh-status');
            if (el) { el.textContent = '✅ ' + bhSlots.length + ' slots'; el.style.color = '#00ff88'; }
            var ct = document.getElementById('bh-slots-ct');
            if (ct) ct.textContent = '(' + bhSlots.length + ')';
          }
        } catch(e) {}
      }, onerror: function() {
        var el = document.getElementById('bh-status');
        if (el) { el.textContent = '⚠️ API indisponible'; el.style.color = '#ff4444'; }
      }
    });
  }

  function bhBuildWindow() {
    if (bhWinBuilt) return;
    bhWinBuilt = true;
    // Styles
    var styleEl = document.createElement('style');
    styleEl.textContent = BH_STYLES;
    ...
    win.innerHTML = [
      '<div id="bh-titlebar">',
        '<div id="bh-titlebar-title">🎰 Bonus Hunt <span id="bh-status" style="font-size:11px;color:#8899aa;font-weight:400;margin-left:8px">⏳ Chargement…</span></div>',
        '<div id="bh-titlebar-btns">',
          '<button class="bh-tbtn" id="bh-min-btn">—</button>',
          '<button class="bh-tbtn" id="bh-close-btn">✕</button>',
        '</div>',
      '</div>',
      '<div id="bh-content">',
        '<div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;flex-wrap:wrap">',
          '<span style="font-size:12px;color:#8899aa">Montant de départ :</span>',
          '<input class="bh-inp" id="bh-start-inp" type="number" value="200" min="0" step="1" style="width:90px" />',
          '<select class="bh-inp" id="bh-currency-sel" style="width:65px">',
            '<option>€</option><option>$</option><option>CA$</option><option>£</option><option>R$</option><option>A$</option>',
          '</select>',
          '<button class="bh-btn2 danger" id="bh-reset-btn" style="margin-left:auto;font-size:11px;padding:4px 10px">Remettre à zéro</button>',
        '</div>',
        '<div class="bh-stat-grid">',
          '<div class="bh-stat-card"><div class="bh-stat-label">Montant de départ</div><div class="bh-stat-val" id="bh-s-start">200,00 €</div></div>',
          '<div class="bh-stat-card"><div class="bh-stat-label">Total gagné</div><div class="bh-stat-val green" id="bh-s-won">0,00 €</div></div>',
          '<div class="bh-stat-card"><div class="bh-stat-label">Profit / Pertes</div><div class="bh-stat-val red" id="bh-s-profit">-200,00 €</div></div>',
          '<div class="bh-stat-card"><div class="bh-stat-label">Slots</div><div class="bh-stat-val purple" id="bh-s-slots">0 <span style="font-size:12px;color:#8899aa">0 att. · 0 col.</span></div></div>',
        '</div>',
        '<div class="bh-two-col">',
          '<div class="bh-box">',
            '<div class="bh-box-title">Break even</div>',
            '<div class="bh-be-grid">',
              '<div class="bh-be-item"><div class="bh-be-label">Fixe</div><div class="bh-be-val" id="bh-be-fixe">0,0×</div></div>',
              '<div class="bh-be-item"><div class="bh-be-label">Évolutif</div><div class="bh-be-val" id="bh-be-evol">0,0×</div></div>',
            '</div>',
          '</div>',
          '<div class="bh-box">',
            '<div class="bh-box-title">🏆 Remarquables</div>',
            '<div id="bh-top" style="font-size:12px;color:#8899aa">Aucun bonus collecté</div>',
          '</div>',
        '</div>',
        '<div class="bh-sec-title">Ajouter un bonus</div>',
        '<div class="bh-form-row">',
          '<div class="bh-ac-wrap">',
            '<div class="bh-lbl">Nom du jeu <span id="bh-slots-ct" style="color:#8899aa"></span></div>',
            '<input class="bh-inp" id="bh-game-inp" type="text" placeholder="Rechercher…" autocomplete="off" />',
            '<div class="bh-ac-list" id="bh-ac-list" style="display:none"></div>',
          '</div>',
          '<div><div class="bh-lbl">Provider</div><input class="bh-inp" id="bh-prov-inp" type="text" placeholder="Auto" readonly /></div>',
          '<div><div class="bh-lbl">Mise</div><input class="bh-inp" id="bh-bet-inp" type="number" value="1" min="0.01" step="0.01" /></div>',
          '<div style="display:flex;flex-direction:column;justify-content:flex-end"><button class="bh-btn2 primary" id="bh-add-btn">+ Ajouter</button></div>',
        '</div>',
        '<div class="bh-sec-title">Tableau des bonus</div>',
        '<table class="bh-table">',
          '<thead><tr><th>#</th><th>Jeu</th><th>Provider</th><th>Mise</th><th>Gain</th><th>Multiplicateur</th><th>Statut</th><th></th></tr></thead>',
          '<tbody id="bh-tbody"><tr><td colspan="8" class="bh-empty-row">Aucun bonus ajouté</td></tr></tbody>',
        '</table>',
      '</div>',
      '<div id="bh-resize">◢</div>',
    ].join('');
    document.body.appendChild(win);
    // Position initiale
    win.style.top = '60px';
    win.style.left = (window.innerWidth - 720) / 2 + 'px';
    // Drag
    bhMakeDraggable(win, document.getElementById('bh-titlebar'));
    // Resize
    bhMakeResizable(win, document.getElementById('bh-resize'));
    // Boutons titre
    document.getElementById('bh-min-btn').addEventListener('click', function() {
      bhMinimized = !bhMinimized;
      document.getElementById('bh-content').classList.toggle('bh-minimized', bhMinimized);
      document.getElementById('bh-min-btn').textContent = bhMinimized ? '□' : '—';
    });
    document.getElementById('bh-close-btn').addEventListener('click', bhCloseWindow);
    document.getElementById('bh-reset-btn').addEventListener('click', function() {
      if (!confirm('Remettre le bonus hunt à zéro ?')) return;
      bhBonuses = []; bhNextId = 1; bhRender();
    });
    // Input start/currency
    document.getElementById('bh-start-inp').addEventListener('input', bhRender);
    document.getElementById('bh-currency-sel').addEventListener('change', bhRender);
    // Autocomplete
    var gameInp = document.getElementById('bh-game-inp');
    gameInp.addEventListener('input', bhGameInput);
    gameInp.addEventListener('focus', bhGameInput);
    gameInp.addEventListener('blur', function(){ setTimeout(function(){ var l=document.getElementById('bh-ac-list'); if(l) l.style.display='none'; }, 200); });
    // Add
    document.getElementById('bh-add-btn').addEventListener('click', bhAdd);
    // Initial render
    bhRender();
  }

  function bhOpenWindow() {
    bhBuildWindow();
    bhLoadSlots();
    var win = document.getElementById('bh-win');
    win.classList.add('bh-visible');
  }

  function bhCloseWindow() {
    var win = document.getElementById('bh-win');
    if (win) win.classList.remove('bh-visible');
  }

  function bhMakeDraggable(el, handle) {
    ...
    function onDrag(e) {
      el.style.left = Math.max(0, ox + e.clientX - mx) + 'px';
      el.style.top  = Math.max(0, oy + e.clientY - my) + 'px';
    }
  }

  function bhMakeResizable(el, handle) {
    handle.addEventListener('mousedown', function(e) {
      e.preventDefault();
      var sw = el.offsetWidth, sh = el.offsetHeight, sx = e.clientX, sy = e.clientY;
      document.addEventListener('mousemove', onResize);
      document.addEventListener('mouseup', function(){ document.removeEventListener('mousemove', onResize); }, {once:true});
      function onResize(e) {
  ...
  function bhGameInput() {
    var q = (document.getElementById('bh-game-inp').value || '').toLowerCase().trim();
    var list = document.getElementById('bh-ac-list');
    ...
    list.innerHTML = matches.map(function(s) {
      ...
      var img = s.thumbnailUrl
        ? '<img class="bh-ac-img" src="' + s.thumbnailUrl + '" alt="" onerror="this.style.display=\'none\'">'  
        : '<div class="bh-ac-img" style="display:flex;align-items:center;justify-content:center;font-size:14px">🎰</div>';
      return '<div class="bh-ac-item" data-name="' + s.name.replace(/"/g,'&quot;') + '" data-prov="' + p.replace(/"/g,'&quot;') + '" data-thumb="' + (s.thumbnailUrl||'').replace(/"/g,'&quot;') + '">' +
        img + '<div><div class="bh-ac-name">' + s.name + '</div><div class="bh-ac-provider">' + p + '</div></div></div>';
    }).join('');
    list.querySelectorAll('.bh-ac-item').forEach(function(item) {
      item.addEventListener('mousedown', function(e) {
        e.preventDefault();
        document.getElementById('bh-game-inp').value = item.getAttribute('data-name');
        document.getElementById('bh-prov-inp').value = item.getAttribute('data-prov');
        list.style.display = 'none';
        bhSelectedThumb = item.getAttribute('data-thumb') || '';
      });
    });
  ...
  function bhAdd() {
    var game = document.getElementById('bh-game-inp').value.trim();
    var prov = document.getElementById('bh-prov-inp').value.trim() || '—';
    var bet  = parseFloat(document.getElementById('bh-bet-inp').value) || 0;
    if (!game) { document.getElementById('bh-game-inp').focus(); return; }
    bhBonuses.push({ id: bhNextId++, game: game, provider: prov, bet: bet, gain: null, done: false, thumb: bhSelectedThumb });
    document.getElementById('bh-game-inp').value = '';
    document.getElementById('bh-prov-inp').value = '';
    bhSelectedThumb = '';
    bhRender();
  }

  function bhRender() {
    var start = parseFloat(document.getElementById('bh-start-inp') ? document.getElementById('bh-start-inp').value : 200) || 0;
    var collected = bhBonuses.filter(function(b){ return b.done; });
    var pending   = bhBonuses.filter(function(b){ return !b.done; });
    var totalWon  = collected.reduce(function(s,b){ return s+(b.gain||0); }, 0);
    var profit    = totalWon - start;
    var n = bhBonuses.length;
    var totalBet  = bhBonuses.reduce(function(s,b){ return s+b.bet; }, 0);
    var avgBet    = n > 0 ? totalBet / n : 0;

    var sStart  = document.getElementById('bh-s-start');  if(sStart)  sStart.textContent = bhFmt(start);
    var sWon    = document.getElementById('bh-s-won');     if(sWon)  { sWon.textContent = bhFmt(totalWon); sWon.className = 'bh-stat-val '+(totalWon>=start?'green':'red'); }
    var sProfit = document.getElementById('bh-s-profit');  if(sProfit){ sProfit.textContent = (profit>=0?'+':'')+bhFmt(profit); sProfit.className = 'bh-stat-val '+(profit>=0?'green':'red'); }
    var sSlots  = document.getElementById('bh-s-slots');   if(sSlots)  sSlots.innerHTML = n+' <span style="font-size:12px;color:#8899aa">'+pending.length+' att. · '+collected.length+' col.</span>';

    var beFix  = avgBet>0&&n>0 ? start/(avgBet*n) : 0;
    var beEvol = pending.length>0&&avgBet>0 ? (start-totalWon)/(avgBet*pending.length) : beFix;
    var beFixEl  = document.getElementById('bh-be-fixe');  if(beFixEl)  beFixEl.textContent  = bhFmtX(beFix);
    var beEvolEl = document.getElementById('bh-be-evol');  if(beEvolEl) beEvolEl.textContent = bhFmtX(Math.max(0,beEvol));

    var top = collected.slice().sort(function(a,b){ return (b.gain||0)-(a.gain||0); }).slice(0,3);
    var topEl = document.getElementById('bh-top');
    if (topEl) topEl.innerHTML = top.length ? top.map(function(b,i){
      var img = b.thumb ? '<img src="'+b.thumb+'" style="width:16px;height:16px;border-radius:2px;object-fit:cover;vertical-align:middle;margin-right:4px" alt="">' : '';
      return '<div style="display:flex;justify-content:space-between;padding:2px 0"><span>'+['🥇','🥈','🥉'][i]+' '+img+b.game+'</span><span style="font-weight:700">'+bhFmtX((b.gain||0)/b.bet)+' → '+bhFmt(b.gain||0)+'</span></div>';
    }).join('') : '<span style="color:#8899aa">Aucun bonus collecté</span>';

    var tbody = document.getElementById('bh-tbody');
    if (!tbody) return;
    if (!bhBonuses.length) { tbody.innerHTML = '<tr><td colspan="8" class="bh-empty-row">Aucun bonus ajouté</td></tr>'; return; }
    tbody.innerHTML = bhBonuses.map(function(b,i){
      var imgTd = b.thumb ? '<img class="bh-slot-img" src="'+b.thumb+'" alt="" onerror="this.style.display=\'none\'">' : '🎰 ';
      var gainTd = b.done
        ? '<span class="bh-gain-done" data-id="'+b.id+'" style="cursor:pointer;'+(b.gain>b.bet?'color:#00ff88':'color:#ff4444')+'" title="Cliquer pour modifier">'+bhFmt(b.gain)+'</span>'
        : '<input class="bh-inp bh-gain-inp" data-id="'+b.id+'" type="number" min="0" step="0.01" placeholder="Gain…" style="width:100px;padding:4px 8px;font-size:12px" />';
      var multi = b.done ? bhFmtX((b.gain||0)/b.bet) : '—';
      var badge = b.done ? '<span class="bh-badge done">✓ Collecté</span>' : '<span class="bh-badge pending">⏳ En attente</span>';
      return '<tr><td style="color:#8899aa">'+(i+1)+'</td>'+
        '<td><span style="font-weight:600">'+imgTd+b.game+'</span></td>'+
        '<td style="color:#8899aa">'+b.provider+'</td>'+
        '<td>'+bhFmt(b.bet)+'</td>'+
        '<td>'+gainTd+'</td>'+
        '<td style="color:#8899aa">'+multi+'</td>'+
        '<td>'+badge+'</td>'+
        '<td><button class="bh-btn2 danger bh-rem-btn" data-id="'+b.id+'" style="padding:3px 8px;font-size:11px">✕</button></td></tr>';
    }).join('');

    tbody.querySelectorAll('.bh-gain-inp').forEach(function(inp){
      function saveGain(){
        if(inp.value==='') return;
      ...
      inp.addEventListener('keydown',function(e){ if(e.key==='Enter') saveGain(); });
      inp.addEventListener('blur', saveGain);
    });
    tbody.querySelectorAll('.bh-gain-done').forEach(function(span){
      span.addEventListener('click',function(){
        var id=parseInt(span.getAttribute('data-id'));
        bhBonuses=bhBonuses.map(function(b){ return b.id===id?Object.assign({},b,{done:false}):b; });
        bhRender();
        setTimeout(function(){ var inp=tbody.querySelector('.bh-gain-inp[data-id="'+id+'"]'); if(inp) inp.focus(); },50);
      });
    });
    tbody.querySelectorAll('.bh-rem-btn').forEach(function(btn){
      btn.addEventListener('click',function(){
  ...
  var CURRENT_VERSION = '1.1.0'; // Doit correspondre a @version
