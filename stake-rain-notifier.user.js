...
(function () {
  ...
  function scanForBet(obj) {
    ...
    // Nom du jeu
    var game = extractGame(obj) || 'Inconnu';
    // Normalise le nom (retire suffixes slug type "-limbo", capitalize)
    ...
    save(SK_WAGER, wager);
    console.log('[StakePulse] Bet WS:', game, '| mise:', amount, '| payout:', payout, '| profit:', profit.toFixed(4));
    if (curTab === 'wager') renderWager();
  }

  function walkForBet(data) {
    if (!data || typeof data !== 'object') return;
    scanForBet(data);
    var vals = Object.values(data);
    for (var i = 0; i < vals.length; i++) {
      if (vals[i] && typeof vals[i] === 'object') walkForBet(vals[i]);
    }
  }
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.0.1'; // Doit correspondre a @version
  ...
  function renderWager() {
    ...
    // Stats
    var totalMise   = filtered.reduce(function(s,w){ return s + (w.amount||0); }, 0);
    var totalProfit = filtered.reduce(function(s,w){ return s + (w.profit||0); }, 0);
    ...
    el.innerHTML = [
      ...
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:6px">',
        '<div class="srn-stat-card"><div class="srn-stat-card-title">Profit</div><div class="srn-stat-card-val" style="color:' + profitColor + '">' + (totalProfit>=0?'+':'') + totalProfit.toFixed(2) + '\u20ac</div></div>',
        '<div class="srn-stat-card"><div class="srn-stat-card-title">Mis\u00e9</div><div class="srn-stat-card-val">' + totalMise.toFixed(2) + '\u20ac</div></div>',
