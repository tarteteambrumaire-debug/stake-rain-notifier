...
// @version      1.4.6
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.4.6 - Filtrage par chat
// @author       alleluiateam
        ...
        if (data[key] !== undefined && data[key] !== null) {
          ...
          if (Array.isArray(data[key]) && Array.isArray(local)) {
            ...
            data[key].forEach(function(e) {
              // Filtrer par chatId si le chat préféré est défini et que l'entrée a un chatId
              if (chatPref && e.chatId && e.chatId !== chatPref) return;
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.4.6'; // Doit correspondre a @version
  ...
  window._srn = {
    ...
    saveConfig: function() {
      ...
      if (chatChanged) {
        // Nettoyer RAIN_LOG
        var log = load(SK.RAIN_LOG, []);
        save(SK.RAIN_LOG, log.filter(function(e) { return !e.chatId || e.chatId === newChatPref; }));
        // Nettoyer SK_RAINERS
        var rainers = load(SK_RAINERS, []);
        save(SK_RAINERS, rainers.filter(function(e) { return !e.chatId || e.chatId === newChatPref; }));
        // Reset rankings (seront recalculés depuis le nouveau RAIN_LOG)
        save(SK.RANKINGS, {});
