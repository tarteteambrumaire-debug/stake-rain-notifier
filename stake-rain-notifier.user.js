...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.5.11'; // Doit correspondre a @version
      ...
      '<div style="display:flex;flex-direction:column;gap:5px;margin-bottom:8px">',
        '<select id="srn-multip-provider" style="background:#162330;border:1px solid #1e3a4a;border-radius:6px;color:#fff;padding:4px 8px;font-size:11px"><option value="">-- Provider --</option>' + (function() {
          ...
          bhSlots.forEach(function(s) {
            var p = (s.provider && (s.provider.label || s.provider.displayName || s.provider.name)) || '';
            // Nettoyer le slug si nécessaire
            p = p.replace(/^game_kurator_group\./, '').replace(/_/g, ' ').replace(/\b\w/g, function(c) { return c.toUpperCase(); });
            if (p && providers.indexOf(p) < 0) providers.push(p);
          });
          providers.sort();
          return providers.map(function(p) { return '<option value="' + escHtml(p) + '">' + escHtml(p) + '</option>'; }).join('');
        })() + '</select>',
