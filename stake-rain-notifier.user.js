...
// @version      1.4.18
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.4.18 - Classement depuis server.js
// @author       alleluiateam
...
(function () {
  ...
  var FIREBASE_KEYS = ['srn_rain_log', 'srn_rankings', 'srn_wordcount', 'srn_emoji_count', 'srn_multip', 'srn_wager', 'srn_rainers'];
  ...
  function fetchLocalRankings(chatId, period, callback) {
    fetch('http://localhost:3002/rankings?chatId=' + encodeURIComponent(chatId) + '&period=' + period)
      .then(function(r) { return r.json(); })
      .then(function(data) { callback(null, data.rankings || []); })
...
'.srn-tick-down{color:#ff4444}',
  ...
  function renderRanking() {
    ...
    function makeRow(p, i, isMe) {
      ...
      var amtStr = '';
      if (p.usdTotal > 0) amtStr = '$' + p.usdTotal.toFixed(2);
      else if (curPref === 'eur' && p.totalEur > 0) amtStr = p.totalEur.toFixed(2) + '\u20ac';
      else if (p.totalUsd > 0) amtStr = '$' + p.totalUsd.toFixed(2);
      else if (p.totalAmount > 0) amtStr = p.totalAmount.toFixed(4) + ' ' + (p.currency || '');
    ...
    // Si bridge connecté → classement depuis server.js (rains.json = même source que @StakeRainLive)
    if (bridgeConnected && chatPref) {
      fetchLocalRankings(chatPref, rankPeriod, function(err, rankings) {
        if (!err && rankings && rankings.length > 0) {
          renderWithData(rankings, rankings);
        } else {
          // Fallback Firebase si pas encore de données locales
  ...
  var CURRENT_VERSION = '1.4.18'; // Doit correspondre a @version
