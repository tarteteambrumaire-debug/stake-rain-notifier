// ==UserScript==
// @name         Stake Rain Notifier
// @namespace    https://stake.bet/
// @version      5.1.3
// @description  Rain notifier complet pour Stake.bet + Multiplicateur tracker
// @author       Vous
...
(function () {
  ...
  var WORD_RULES = [
    { id: 'reglement',  label: 'Reglement',  pattern: /stakecommunity\.com\/topic\/981/i, senderFilter: 'gabifrkr' },
    ...
    { id: 'restetchat', label: 'Reste tchat',pattern: /reste dans ton.{0,5}chat/i, senderFilter: null },
  ...
  var CONFIG = {
    ...
    YOUR_USERNAME:      'alleluiateam',
    WATCH_KEYWORDS:     ['alleluiateam'],
  };
  ...
  function fbPush() {
    ...
    console.log('[RainNotifier] Sync Firebase OK');
  }
  ...
  function fbPull(callback) {
    fbGet('shared', function(data) {
      ...
      FIREBASE_KEYS.forEach(function(key) {
        if (data[key] !== undefined && data[key] !== null) {
          ...
          if (Array.isArray(data[key]) && Array.isArray(local)) {
            // Merge les tableaux par ts unique
            ...
            // Merge les objets (classement, emojis, wc)
      ...
      console.log('[RainNotifier] Pull Firebase OK');
  ...
  function trackWordCount(text, sender) {
    // Deduplication persistante : survit aux refreshs (TTL 2h)
  ...
  function recordRain(sender, parsed) {
    // Deduplication : ignore si rain identique dans les 10 secondes
  ...
  function scanPayload(data) {
    walkObject(data, function(obj) {
      ...
      var sender = obj.username || obj.name || (obj.user && (obj.user.name || obj.user.username)) || (obj.author && (obj.author.name || obj.author.username)) || obj.sender || null;
  ...
  window.fetch = function() {
    ...
    return _fetch.apply(this, args).then(function(res) {
      try {
        ...
        if (url.indexOf('graphql') >= 0 || url.indexOf('chat') >= 0) res.clone().json().then(scanPayload).catch(function(){});
      } catch(e) {}
      return res;
    });
  };
  ...
  // Capture les bets depuis le tableau en bas de page
  var lastBetKey = '';
    ...
    new MutationObserver(function(mutations) {
      mutations.forEach(function(m) {
        m.addedNodes.forEach(function(node) {
          ...
          // Ignore si on est sur la page d'accueil (pas une page de jeu)
          var path = window.location.pathname;
          if (path === '/fr' || path === '/fr/' || path === '/' || path === '') return;
          var text = (node.textContent || '').trim();
          if (text.length < 5 || text.length > 200) return;
          // Cherche un montant en euros dans le texte (ex: 0.37 apres euro)
          var euroMatches = text.match(/[€€]([\d]+[.,][\d]+)/g);
          if (!euroMatches || !euroMatches.length) return;
          // Ignore si c'est juste un prix affiche (pas un resultat de bet)
          // Un bet a toujours : heure (PM/AM ou HH:MM) + montant
          ...
          // Extrait le premier montant euro = la mise
          var firstMatch = text.match(/€([\d]+[.,][\d]+)/);
          if (!firstMatch) return;
          var amount = parseFloat(firstMatch[1].replace(',', '.'));
          if (amount <= 0 || amount > 5000) return; // Ignore les montants trop eleves (paris publics)
          // Enregistre le bet
          var wager = load(SK_WAGER, []);
          wager.push({ ts: Date.now(), amount: amount });
          var twoYears = Date.now() - 2*365*24*3600*1000;
          wager = wager.filter(function(w) { return w.ts > twoYears; });
          save(SK_WAGER, wager);
          console.log('[RainNotifier] Bet detecte:', amount, 'EUR |', text.substring(0,60));
          if (curTab === 'wager') renderWager();

          // ── Extraction du multiplicateur depuis le texte du bet ──
          // Format observe : "Crash10:41 PM €0.01   2,00× ×€0.02"
          // ou              : "Plinko10:42 PM €0.01   15,50× ×€0.15"
          var multMatch = text.match(/([\d]+[,.][\d]+)\s*[×x✕]/i);
          if (multMatch) {
            var multVal = parseFloat(multMatch[1].replace(',', '.'));
            // Extrait le nom du jeu : tout ce qui precede l'heure (HH:MM ou digits:digits)
            ...
            if (isFinite(multVal) && multVal >= 1.01) {
              console.log('[RainNotifier] Multiplicateur DOM:', multVal, 'jeu:', gameName);
  ...
  // Config par defaut : seuil global et liste de jeux avec seuil specifique
  ...
  function getMultipThresholdForGame(gameName) {
    ...
    // Cherche d'abord un seuil specifique pour ce jeu
  ...
  function checkMultiplier(gameName, multiplier) {
    ...
    showNotif(msg);
    console.log('[RainNotifier] Multiplicateur :', multiplier, 'sur', gameName, '| Seuil :', threshold);
  ...
  function scanForMultiplier(obj) {
    ...
    if (MULTIP_DEBUG) {
      console.log('[RainNotifier][MULTIP DEBUG] field=' + foundField + ' mult=' + mult + ' game=' + game, JSON.stringify(obj).substring(0, 200));
    }
  ...
  window._srnEnableMultipDebug = function() {
    MULTIP_DEBUG = true;
    console.log('[RainNotifier] Mode debug multiplicateur ACTIVE. Joue un bet pour voir les payloads.');
  };
  window._srnDisableMultipDebug = function() {
    MULTIP_DEBUG = false;
    console.log('[RainNotifier] Mode debug multiplicateur desactive.');
  };
  ...
  function trackEmojis(node) {
    ...
    // Cherche toutes les imgs dont l'alt ressemble a :nom: (emojis Stake)
    ...
    emojiImgs.forEach(function(img) {
      ...
      changed = true;
      console.log('[RainNotifier] Emoji detecte:', alt);
    });
    ...
    new MutationObserver(function(mutations) {
      mutations.forEach(function(mutation) {
        mutation.addedNodes.forEach(function(node) {
          ...
          if (hasUserTags || hasKeyword || looksLikeRain(text)) processMessage(text, sender);
          trackEmojis(node);
          trackWordCount(text, sender);
        });
      });
  ...
  function showNotif(msg) {
    if (Notification.permission === 'granted')
      new Notification('Stake Rain Notifier', { body: msg, icon: 'https://stake.bet/favicon.ico' });
  }
...
'.srn-tick-down{color:#ff4444}',
  ...
  function buildPanel() {
    ...
    panelEl.innerHTML = [
      '<div id="srn-hdr">',
        '<span>&#127783; Rain Notifier <span id="srn-notif-badge" style="display:none;background:#ff4444;color:#fff;border-radius:999px;font-size:10px;padding:1px 6px;margin-left:4px;font-weight:700;vertical-align:middle"></span></span>',
      ...
      '<div id="srn-body">',
        '<div class="srn-tabs">',
          '<button class="srn-tab active" data-tab="dashboard">Stats</button>',
          '<button class="srn-tab" data-tab="ranking">Classement</button>',
          '<button class="srn-tab" data-tab="history">Historique</button>',
          '<button class="srn-tab" data-tab="messages">Messages</button>',
          '<button class="srn-tab" data-tab="mystats">Mes Stats</button>',
          '<button class="srn-tab" data-tab="moderation">Mots</button>',
          '<button class="srn-tab" data-tab="rainers">Rains</button>',
          '<button class="srn-tab" data-tab="wager">Stats Wager</button>',
          '<button class="srn-tab" data-tab="multip">Multiplicateur</button>',
          '<button class="srn-tab" data-tab="converter">Convertisseur</button>',
          '<button class="srn-tab" data-tab="rang">Rang</button>',
          '<button class="srn-tab" data-tab="settings">Config</button>',
        '</div>',
        ...
        '<div class="srn-sec active" id="tab-dashboard">',
          ...
          '<button class="srn-btn p" data-srn="sendRank-weekly">Classement semaine -> Telegram</button>',
          '<button class="srn-btn p" style="margin-top:5px" data-srn="sendRank-monthly">Classement mois -> Telegram</button>',
          '<button class="srn-btn p" style="margin-top:5px" data-srn="sendRank-allTime">Classement all-time -> Telegram</button>',
        '</div>',
        ...
        '<div class="srn-sec" id="tab-settings">',
          ...
          '<div class="srn-cfg-r"><label>Mon pseudo</label><input type="text" id="cfg-usr" placeholder="monpseudo" /></div>',
          ...
          '<div class="srn-kw-add"><input type="text" id="cfg-kw-input" placeholder="ex: alleluiateam" /><button data-srn="addKeyword">+ Ajouter</button></div>',
          '<div class="srn-sec-title" style="margin-top:12px">Synchronisation Firebase</div>',
          '<div style="display:flex;gap:5px;margin-bottom:6px">',
            '<button class="srn-btn p" style="margin-top:0;flex:1" data-srn="fbPullNow">&#8595; Recuperer les stats</button>',
            '<button class="srn-btn p" style="margin-top:0;flex:1" data-srn="fbPushNow">&#8593; Envoyer mes stats</button>',
          '</div>',
          '<div id="srn-fb-status" style="font-size:10px;color:#8899aa;text-align:center;margin-bottom:8px"></div>',
    ...
    panelEl.querySelectorAll('.srn-tab').forEach(function(btn) {
      btn.addEventListener('click', function() {
        curTab = btn.getAttribute('data-tab');
        panelEl.querySelectorAll('.srn-tab').forEach(function(b) { b.classList.remove('active'); });
    ...
    panelEl.addEventListener('click', function(e) {
      ...
      else if (action === 'saveOffset')       srn.saveOffset();
  ...
  function applyTabsConfig() {
    ...
    ALL_TABS.forEach(function(t) {
      var btn = panelEl.querySelector('[data-tab="' + t.id + '"]');
      if (btn) btn.style.display = cfg[t.id] ? '' : 'none';
      // Si l'onglet actif est masque, aller sur Stats
      if (!cfg[t.id] && curTab === t.id) {
        curTab = 'dashboard';
        panelEl.querySelectorAll('.srn-tab').forEach(function(b) { b.classList.remove('active'); });
        var dash = panelEl.querySelector('[data-tab="dashboard"]');
  ...
  function refreshPanel() {
    ...
    var msgTab = panelEl.querySelector('[data-tab="messages"]');
    if (msgTab) msgTab.textContent = unread > 0 ? 'Messages (' + unread + ')' : 'Messages';
  ...
  function renderMyStats() {
    ...
    el.innerHTML = [
      ...
      '</div>',
      '<div class="srn-sec-title">Correction manuelle</div>',
      '<div style="display:flex;gap:5px;margin-bottom:6px">',
        '<input type="text" id="srn-offset-month" placeholder="Mois EUR" style="flex:1;background:#162330;border:1px solid #1e3a4a;border-radius:6px;color:#fff;padding:4px 8px;font-size:11px" />',
        '<input type="text" id="srn-offset-month-count" placeholder="Mois rains" style="flex:1;background:#162330;border:1px solid #1e3a4a;border-radius:6px;color:#fff;padding:4px 8px;font-size:11px" />',
      '</div>',
      '<div style="display:flex;gap:5px;margin-bottom:8px">',
        '<input type="text" id="srn-offset-alltime" placeholder="All-time EUR" style="flex:1;background:#162330;border:1px solid #1e3a4a;border-radius:6px;color:#fff;padding:4px 8px;font-size:11px" />',
        '<input type="text" id="srn-offset-alltime-count" placeholder="All-time rains" style="flex:1;background:#162330;border:1px solid #1e3a4a;border-radius:6px;color:#fff;padding:4px 8px;font-size:11px" />',
      '</div>',
      '<div style="display:flex;gap:5px;margin-bottom:6px">',
        '<input type="text" id="srn-offset-week" placeholder="Semaine EUR (ex: 29.31)" style="flex:1;background:#162330;border:1px solid #1e3a4a;border-radius:6px;color:#fff;padding:4px 8px;font-size:11px" />',
      '</div>',
      '<button class="srn-btn p" style="margin-bottom:10px" data-srn="saveOffset">Sauvegarder la correction</button>',
  ...
  function renderWager() {
    ...
    var total    = filtered.reduce(function(s, w) { return s + w.amount; }, 0);
    var count    = filtered.length;
    var avgBet   = count > 0 ? total / count : 0;
    var maxBet   = count > 0 ? Math.max.apply(null, filtered.map(function(w) { return w.amount; })) : 0;
    var periodLabel = { day: "aujourd'hui", week: 'cette semaine', month: 'ce mois', year: 'cette annee' }[wagerPeriod];
    // Graphique par heure (jour) ou par jour (autres)
    var buckets = [];
    if (wagerPeriod === 'day') {
      for (var h = 0; h < 24; h++) {
        var hStart = dayStart + h*3600*1000, hEnd = hStart + 3600*1000;
        var hTotal = filtered.filter(function(w){return w.ts>=hStart&&w.ts<hEnd;}).reduce(function(s,w){return s+w.amount;},0);
        buckets.push({ label: h+'h', total: hTotal });
      }
    } else {
      var days = wagerPeriod==='week'?7:wagerPeriod==='month'?30:365;
      for (var d = days-1; d >= 0; d--) {
        var dStart = now - d*24*3600*1000; dStart = new Date(dStart).setHours(0,0,0,0);
        var dEnd = dStart + 24*3600*1000;
        var dTotal = filtered.filter(function(w){return w.ts>=dStart&&w.ts<dEnd;}).reduce(function(s,w){return s+w.amount;},0);
        var dDate = new Date(dStart);
        buckets.push({ label: dDate.getDate()+'/'+(dDate.getMonth()+1), total: dTotal });
      }
    }
    var maxB = Math.max.apply(null, buckets.map(function(b){return b.total;})) || 1;
    // Afficher seulement les buckets non vides pour le graphique (max 12)
    var visibleBuckets = buckets.filter(function(b){return b.total>0;}).slice(-12);
    if (!visibleBuckets.length) visibleBuckets = buckets.slice(-12);
    var barsHtml = visibleBuckets.map(function(b) {
      var h2 = Math.max(4, Math.round((b.total/maxB)*60));
      return '<div style="display:flex;flex-direction:column;align-items:center;flex:1;gap:2px">'
        + '<div style="font-size:9px;color:#8899aa">' + (b.total>0?b.total.toFixed(1):'') + '</div>'
        + '<div style="width:100%;background:#00ff8833;border-radius:3px 3px 0 0;height:'+h2+'px" title="'+b.total.toFixed(2)+'EUR"></div>'
        + '<div style="font-size:9px;color:#8899aa;white-space:nowrap">' + b.label + '</div>'
        + '</div>';
    }).join('');
    el.innerHTML = [
      '<div class="srn-sec-title">Wager ' + periodLabel + '</div>',
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:10px">',
        '<div class="srn-stat-card"><div class="srn-stat-card-title">Total wage</div><div class="srn-stat-card-val" style="color:#00ff88">' + total.toFixed(2) + '\u20ac</div><div class="srn-stat-card-sub">' + count + ' bet' + (count>1?'s':'') + '</div></div>',
        '<div class="srn-stat-card"><div class="srn-stat-card-title">Mise moyenne</div><div class="srn-stat-card-val">' + avgBet.toFixed(2) + '\u20ac</div><div class="srn-stat-card-sub">par bet</div></div>',
        '<div class="srn-stat-card"><div class="srn-stat-card-title">Plus gros bet</div><div class="srn-stat-card-val">' + maxBet.toFixed(2) + '\u20ac</div></div>',
        '<div class="srn-stat-card"><div class="srn-stat-card-title">Bets detectes</div><div class="srn-stat-card-val">' + count + '</div></div>',
      '</div>',
      '<div class="srn-sec-title">Evolution</div>',
      '<div style="display:flex;align-items:flex-end;gap:3px;height:80px">' + barsHtml + '</div>',
      count === 0 ? '<div class="srn-empty" style="margin-top:8px">Aucun bet detecte. Joue sur Stake pour voir tes stats !</div>' : '',
    ].join('');
  }
  ...
  window._srn = {
    ...
    saveConfig: function() {
      ...
      if (usr) CONFIG.YOUR_USERNAME      = usr;
    ...
    saveOffset: function() {
      var month      = parseFloat(document.getElementById('srn-offset-month').value) || 0;
      var monthCount = parseInt(document.getElementById('srn-offset-month-count').value) || 0;
      var allTime    = parseFloat(document.getElementById('srn-offset-alltime').value) || 0;
      var allTimeCount = parseInt(document.getElementById('srn-offset-alltime-count').value) || 0;
      var week = parseFloat(document.getElementById('srn-offset-week').value) || 0;
      save(SK_MYSTATS_OFFSET, { month: month, monthCount: monthCount, allTime: allTime, allTimeCount: allTimeCount, week: week });
      renderMyStats();
    },
    ...
    goToMessages: function() {
      ...
      curTab = 'messages';
      panelEl.querySelectorAll('.srn-tab').forEach(function(b) { b.classList.remove('active'); });
      var t = panelEl.querySelector('[data-tab="messages"]'); if (t) t.classList.add('active');
      panelEl.querySelectorAll('.srn-sec').forEach(function(s) { s.classList.remove('active'); });
  ...
  function init() {
    ...
    var start = function() {
      ...
      observeBets(); // Observe les bets en temps reel
      setInterval(refreshPanel, 30000);
      if (Notification.permission === 'default') Notification.requestPermission();
      console.log('[RainNotifier] Actif sur Stake.bet | Pseudo:', CONFIG.YOUR_USERNAME, '| Mots-cles:', CONFIG.WATCH_KEYWORDS);
      fetchCryptoPrices(updateTicker);
      // Sync Firebase au démarrage
      fbPull(function() { refreshPanel(); console.log('[RainNotifier] Donnees Firebase chargees'); });
