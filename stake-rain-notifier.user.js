...
// @version      1.4.1
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.4.1 - Filtrage par chat
// @author       alleluiateam
...
(function () {
  ...
  var CHAT_OPTIONS = [
    { id: 'f0326994-ee9e-411c-8439-b4997c187b95', label: 'EN English' },
  ...
  function recordRain(sender, parsed) {
    var rainKey = (parsed.ts || '') + '|' + (parsed.amount || '') + '|' + (parsed.recipients || []).slice(0,3).join(',');
...
'.srn-tick-down{color:#ff4444}',
  ...
  function refreshPanel() {
    ...
    var log = load(SK.RAIN_LOG, []);
    var chatPref = load(SK_CHAT_PREF, '');
    // Filtre par chat préféré pour les stats locales (Classement, Historique, Mes Stats, Rains)
  ...
  var CURRENT_VERSION = '1.4.1'; // Doit correspondre a @version
  ...
  window._srn = {
    ...
    saveConfig: function() {
      ...
      var chatPrefEl = document.getElementById('cfg-chat-pref');
      if (chatPrefEl && chatPrefEl.value) save(SK_CHAT_PREF, chatPrefEl.value);
      var c = load(SK.USER_CONFIG, {});
