...
(function () {
  ...
  function checkMultiplier(gameName, multiplier) {
    ...
    sendTelegram('\uD83C\uDF1F <b>Multiplicateur atteint !</b>\n\uD83C\uDFB0 Jeu : <b>' + escHtml(gameName || 'Inconnu') + '</b>\n\uD83D\uDCC8 Multiplicateur : <b>x' + multiplier.toFixed(2) + '</b>\n\uD83D\uDEA8 Seuil : x' + threshold + '\n\uD83D\uDD50 ' + new Date().toLocaleTimeString('fr-FR'));
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.5.12'; // Doit correspondre a @version
  ...
  function renderMultip() {
    ...
    el.innerHTML = [
      ...
      '<div style="display:flex;flex-direction:column;gap:5px;margin-bottom:8px">',
        '<select id="srn-multip-provider" style="background:#162330;border:1px solid #1e3a4a;border-radius:6px;color:#fff;padding:4px 8px;font-size:11px"><option value="">-- Provider --</option>' + (function() {
          if (!bhSlots || !bhSlots.length) return '';
          var providers = [];
          bhSlots.forEach(function(s) {
            var rawP = (s.provider && (s.provider.label || s.provider.displayName || s.provider.name)) || '';
            if (rawP && !providers.some(function(p) { return p.raw === rawP; })) {
              var label = rawP.replace(/^game_kurator_group\./, '').replace(/_/g, ' ').replace(/\b\w/g, function(c) { return c.toUpperCase(); });
              providers.push({ raw: rawP, label: label });
            }
          });
          providers.sort(function(a, b) { return a.label.localeCompare(b.label); });
          return providers.map(function(p) { return '<option value="' + escHtml(p.raw) + '">' + escHtml(p.label) + '</option>'; }).join('');
        })() + '</select>',
    ...
    if (provSel && gameSel && !provSel._hasListener) {
      ...
      provSel.addEventListener('change', function() {
        ...
        if (!prov || !bhSlots || !bhSlots.length) return;
        var games = bhSlots.filter(function(s) {
          var rawP = (s.provider && (s.provider.label || s.provider.displayName || s.provider.name)) || '';
          return rawP === prov;
        });
  ...
  connectBridge();
  interceptWebSocket();
  // Charger les slots au démarrage pour le Multiplicateur
  setTimeout(function() { bhLoadSlots(); }, 3000);
