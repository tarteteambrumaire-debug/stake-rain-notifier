...
'.srn-tick-down{color:#ff4444}',
  ...
  function bhLoadProviders(callback) {
    ...
    fetch(BH_API + '/api/providers')
      ...
      .then(function(d) {
        if (d.success && d.data) {
          bhProviders = d.data.sort(function(a, b) { return a.name.localeCompare(b.name); });
          if (callback) callback();
        }
      })
      .catch(function() {});
  ...
  function bhLoadProviderSlots(providerId, callback) {
    ...
    fetch(BH_API + '/api/providers/' + providerId + '/slots')
      ...
      .then(function(d) {
        if (d.success && d.data) {
          bhProviderSlotsCache[providerId] = d.data.sort(function(a, b) { return a.name.localeCompare(b.name); });
          callback(bhProviderSlotsCache[providerId]);
        } else { callback([]); }
      })
  ...
  var CURRENT_VERSION = '1.5.15'; // Doit correspondre a @version
  ...
  function renderMultip() {
    ...
    if (bhProviders.length === 0) {
      el.innerHTML = '<div style="color:#8899aa;font-size:12px;padding:20px;text-align:center">Chargement des providers...</div>';
      bhLoadProviders(function() { renderMultip(); });
      return;
    }
    var cfg = getMultipConfig();
    var log = load(SK_MULTIP, []);
